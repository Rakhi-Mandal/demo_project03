...
test('generated flow @sanity', async ({ page }) => {
  ...
  await heal(page, 'stop 1 location button', 'visible', null,
    () => page.locator('form[id="stop-1-content-location"] button[type="button"]'));
  ...
  await heal(page, 'date option', 'visible', null,
    () => page.locator('span').filter({ hasText: /^30$/ }).first());
  await heal(page, 'date option', 'click', null,
    () => page.locator('span').filter({ hasText: /^30$/ }).first());

  // 15. Stop 1: Fill internal notes (collapse typing progression)
  await heal(page, 'stop 1 notes field', 'visible', null,
    () => page.locator('#stop-1-content-internal-notes'));
  await heal(page, 'stop 1 notes field', 'fill', testData.stop1ContentInternalNotes,
    () => page.locator('#stop-1-content-internal-notes'));

  // 16. Stop 2: Open location dropdown
  await heal(page, 'stop 2 location button', 'visible', null,
    () => page.locator('form[id="stop-2-content-location"] button[type="button"]'));
  ...
  await heal(page, 'line item 1 product button', 'visible', null,
    () => page.locator('div[id="line-item-num-1-content"] button[type="button"]'));
  await heal(page, 'line item 1 product button', 'click', null,
    () => page.locator('div[id="line-item-num-1-content"] button[type="button"]'));
  ...
  await heal(page, 'bill to location button', 'visible', null,
    () => page.locator('form[id="bill-to-content-location"] button[type="button"]'));
